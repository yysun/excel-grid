// Unit tests for GridStore's live text search: setSearchQuery/setSearchScope,
// isCellMatched, and how searchHiddenRows folds into isRowHidden alongside
// manual hides and column filters.
...
describe("setSearchScope", () => {
  it("'selected' scope only matches within the given columns", () => {
    const s = make();
    seed(s);
    s.setSearchScope("selected", [0]); // column 0 only
  ...
  it("switching back to 'all' re-widens matching", () => {
    const s = make();
    seed(s);
    s.setSearchScope("selected", [0]);
    s.setSearchQuery("banana");
    expect(s.isRowHidden(3)).toBe(true);
  ...
  it("an empty selected-column set has nothing to scan, so no rows hide", () => {
    const s = make();
    seed(s);
    s.setSearchScope("selected", []);
    s.setSearchQuery("apple");
    expect(s.isRowHidden(0)).toBe(false);
    expect(s.isCellMatched(0, 0)).toBe(false);
  });

  it("ignores out-of-range columns in the selected set", () => {
    const s = make();
    seed(s);
    s.setSearchScope("selected", [0, 99, -1]);
    s.setSearchQuery("apple");
    expect(s.isRowHidden(0)).toBe(false);
  });
});
